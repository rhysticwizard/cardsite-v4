...
export async function POST(request: NextRequest) {
  try {
    // Rate limiting - 2 verification email sends per minute per IP
    const clientIP = getClientIP(request)
    const rateLimitResult = checkRateLimit(`send-verification:${clientIP}`, RATE_LIMITS.FORGOT_PASSWORD)
    
    if (!rateLimitResult.allowed) {
      console.log(`🚫 Rate limit exceeded for send verification from IP: ${clientIP}`)
